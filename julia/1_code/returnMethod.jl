...
function eulerLinearized(new_nb_cpoints::Int64, max_nb_cpoints::Int64, 
    ...
    if new_nb_cpoints < 0 || new_nb_cpoints > max_nb_cpoints
        ...
        ## Solvint the system
        Eta_k_current = current_conditions.η_k; 
        u_k_current   = current_conditions.u_k;   
        z_k_current   = current_conditions.z_k; 
        v_k_current   = current_conditions.v_k;
        x = [Eta_k_current;u_k_current; z_k_current;v_k_current]; x = x[(new_nb_cpoints+1):end];
        ...
        Eta_k_probable = [z_k_probable .- (f(0:(new_nb_cpoints-1))); results[1:(Ntot-new_nb_cpoints)]];
        ...
        if new_nb_cpoints != 0
            ...
            approximated_slope = (Eta_k_probable[new_nb_cpoints + 1] - Eta_k_probable[new_nb_cpoints])/dr;
            exact_slope = g(ppt);
